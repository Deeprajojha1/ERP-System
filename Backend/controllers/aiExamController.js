...
/* ================= TEACHER - DOWNLOAD SCORES REPORT ================= */
const deriveGrade = (awarded, max) => {
  if (awarded == null || max == null || max === 0) return "";
...
export const downloadExamScoresReport = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const attempts = await ExamAttempt.find({ blueprintId: id })
      .sort({ createdAt: -1 })
      .populate({
        path: "student",
        select: "enrollmentNumber user",
    ...
    // Build best-attempt-per-student map (highest score)
    const bestByStudent = new Map();
    attempts.forEach((attempt) => {
      const studentId = String(attempt.student?._id || "");
      if (!studentId) return;
      const score = evalMap.get(String(attempt._id));
      const awarded = score?.totalAwarded ?? null;
      const max = score?.totalMax ?? null;
      const existing = bestByStudent.get(studentId);
      if (!existing || (awarded != null && (existing.awarded == null || awarded > existing.awarded))) {
    ...
    const rows = Array.from(bestByStudent.values())
      .map((entry) => ({
        enrollmentNumber: entry.attempt.student?.enrollmentNumber || "",
        name: entry.attempt.student?.user?.name || "N/A",
        marks: entry.awarded != null ? entry.awarded : 0,
        grade: deriveGrade(entry.awarded, entry.max),
    ...
    return res.status(500).json({ message: error.message || "Failed to generate scores report" });
  }
};
