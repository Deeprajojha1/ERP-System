...
const ACADEMIC_YEAR_DEMAND_ERROR_MESSAGE =
  "Invalid academicYear. Use YYYY-YYYY or YYYY-YY. If you pass a batch window like 2024-2028, we will use 2024-2025.";
...
// For demand/payment flows we allow the admin to pass a batch window like 2024-2028.
// We normalize that to the first academic year (2024-2025) because hostel/transport/calendar configs are yearly.
const normalizeAcademicYearForDemand = (value) => {
  const raw = sanitizeText(value, 16);
  const normalized = normalizeAcademicYear(raw, { format: "full" });
...
const autoGenerateDemandsForProfile = async ({ profile, branch, program, academicYear }) => {
  ...
  let normalizedAcademicYear = normalizeAcademicYear(academicYear, { format: "full" });
  if (!normalizedAcademicYear) {
    const start = getAcademicStartYear(academicYear);
    if (Number.isFinite(start)) normalizedAcademicYear = `${start}-${start + 1}`;
  }
...
const resolveFeeReferencesFromStudent = async (studentDoc) => {
  ...
  const branches = await Branch.find({ programId: matchedProgram._id }).select(
    "_id branchName batchYear createdAt semesterBaseFees programId hostelYearlyFee transportYearlyFee"
  );
  const findNameCandidates = () => {
    const normalizeBranch = (b) => normalizeLoose(b?.branchName || "");
    const exact = (norm) => branches.filter((b) => normalizeBranch(b) === norm);
    const includes = (norm) => branches.filter((b) => normalizeBranch(b).includes(norm));

    const primary = groupBranchNorm ? exact(groupBranchNorm) : [];
    if (primary.length) return primary;
    const byDept = exact(departmentNorm);
    if (byDept.length) return byDept;
    const byDeptIncludes = includes(departmentNorm);
    if (byDeptIncludes.length) return byDeptIncludes;
    return [];
  };
  ...
  const targetBatchYear = Number(matchedBatch?.batchYear);
  const pickNewest = (list) =>
    list
      .slice()
      .sort((a, b) => new Date(b?.createdAt || 0).getTime() - new Date(a?.createdAt || 0).getTime())[0] || null;

  const pickBestByBatch = (list) => {
    if (!list.length) return null;
    if (Number.isFinite(targetBatchYear)) {
      const exact = list.filter((b) => Number(b?.batchYear) === targetBatchYear);
      if (exact.length) return pickNewest(exact);

      const earlier = list
        .filter((b) => Number.isFinite(Number(b?.batchYear)) && Number(b.batchYear) <= targetBatchYear)
        .sort((a, b) => {
          const byYear = Number(b.batchYear) - Number(a.batchYear);
    ...
    const legacy = list.filter((b) => b?.batchYear == null);
    if (legacy.length) return pickNewest(legacy);
    return pickNewest(list);
  };

  const matchedBranch = pickBestByBatch(nameCandidates);
...
export const createFeeBranch = async (req, res) => {
  try {
    ...
    if (
      !Number.isFinite(normalizedBatchYear) ||
      normalizedBatchYear < 2000 ||
      normalizedBatchYear > 2100
    ) {
      return res.status(400).json({ message: "batchYear is required (YYYY)" });
    }
    ...
    const branch = existing?._id
      ? await Branch.findByIdAndUpdate(existing._id, updatePayload, { new: true })
      : await Branch.create(updatePayload);

    await Program.findByIdAndUpdate(programId, { $addToSet: { branchIds: branch._id } });
    await logFeeAudit(req, {
      action: existing?._id ? "FEE_BRANCH_UPDATED" : "FEE_BRANCH_CREATED",
      entityType: "Branch",
      entityId: branch._id,
      metadata: {
        programId,
    ...
    return res
      .status(existing?._id ? 200 : 201)
      .json({
        message: existing?._id ? "Fee branch updated successfully" : "Fee branch created successfully",
        data: branch,
      });
  } catch (error) {
    if (String(error?.message || "").includes("_INVALID") || String(error?.message || "").includes("_REQUIRED")) {
...
export const getFeeBranches = async (req, res) => {
  try {
    await ensureProgramsSeeded();
    const { programId, batchYear } = req.query || {};
    const query = {};
    if (programId) {
      if (!isValidId(programId)) return res.status(400).json({ message: "Invalid programId" });
      query.programId = programId;
    }
    if (batchYear != null && String(batchYear).trim() !== "") {
    ...
    const rows = await Branch.find(query)
      ...
      .select("_id programId branchName batchYear hostelYearlyFee transportYearlyFee semesterBaseFees");
...
export const createFeeDemand = async (req, res) => {
  try {
    ...
    const safeStudentId = ensureSafeText(studentId, "student_id");
    const safeAcademicYear = normalizeAcademicYearForDemand(academicYear);
    if (!safeAcademicYear) {
      return res.status(400).json({ message: ACADEMIC_YEAR_DEMAND_ERROR_MESSAGE });
    }
...
export const lookupFeeStudentByEnrollment = async (req, res) => {
  try {
    const enrollmentNumber = String(req.params?.enrollmentNumber || req.query?.enrollmentNumber || "").trim();
    if (!enrollmentNumber) {
    ...
    const student = await Student.findOne({
      ...
      .select("enrollmentNumber fatherName fatherPhoneNumber academicYear collegeEmail program semester department batchId user")
      .lean();

    if (!student?._id) return res.status(404).json({ message: "Student not found" });
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        email: student?.user?.email || student?.collegeEmail || "",
...
export const getPaymentHistory = async (req, res) => {
  try {
    ...
    const rows = await PaymentHistory.find(filter)
      ...
      .populate({
        path: "demandId",
        select: "academicYear semesterNo totalAmount dueAmount studentId studentMongoId",
        populate: {
          path: "studentMongoId",
    ...
    const safeRows = rows.map((item) => {
      ...
      row.studentName = row?.demandId?.studentMongoId?.userId?.name || "";
...
export const createMyRazorpayOrderForYear = async (req, res) => {
  try {
    ...
    const academicYear = normalizeAcademicYearForDemand(String(req.body?.academicYear || "").trim());
    if (!academicYear) {
      return res.status(400).json({ message: ACADEMIC_YEAR_DEMAND_ERROR_MESSAGE });
    }
