...
const FeesHostel = () => {
  ...
  const hostelDemands = useMemo(
    () => demands.filter((d) => (d.breakdown || []).some((b) => b.head === "HOSTEL")),
    [demands]
  );
    ...
    <div className="fees-page">
      <div className="fee-dashboard-headline">
        <div className="fee-dashboard-copy">
          <h1 className="fee-dashboard-title">Hostel Fees</h1>
          <p className="fee-dashboard-context">Real-time hostel fee collection data</p>
        </div>
        <button
          type="button"
      ...
      <div className="fees-summary">
        <div className="fees-card">
          ...
          <span className="fees-card-note">{stats.studentCount} students with hostel fees</span>
        </div>
        <div className="fees-card">
          <div className="fees-card-head">
        ...
        <div className="fees-card">
          ...
          <span className="fees-card-note">Hostel fee recovery</span>
        </div>
      </div>

      <section className="fee-table-section">
        <div className="fee-table-head">
          <h2 className="fee-table-title">Hostel Fee Demands</h2>
        </div>
        <div className="fees-table-wrap">
          <table className="fees-table">
            ...
            <tbody>
              {hostelDemands.map((d) => {
                const hostel = (d.breakdown || []).find((b) => b.head === "HOSTEL");
                return (
                  <tr key={d._id}>
                    <td className="fees-name">{d.studentId}</td>
                    <td>{d.academicYear}</td>
                    <td>{formatSemesterLabel(d.semesterNo, d.scope)}</td>
                    <td>{formatCurrency(hostel?.amount)}</td>
                    <td>{formatCurrency(hostel?.paid)}</td>
                    <td>{d.status}</td>
                  </tr>
                );
              })}
              {hostelDemands.length === 0 && (
                <tr><td colSpan={6}>No hostel fee demands found</td></tr>
              )}
            </tbody>
          </table>
        </div>
