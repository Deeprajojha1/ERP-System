...
import { migrateBase64ProfileImage } from "./profileController.js";
...
/* Fetch role-specific details */
    ...
    if (user.role === "student") {
      ...
      const [
        ...
        // Faculty aggregates (all from User collection for consistency)
        ...
        // Student aggregates (all from User collection for consistency)
    ...
    // Migrate base64 profile image to file if needed
    if (user.profileImage && user.profileImage.startsWith('data:')) {
      await migrateBase64ProfileImage(user);
    }
...
export const createAdmin = async (req, res) => {
  try {
    ...
    // Send welcome email (optional - wrap in try-catch to not block admin creation)
    try {
      await sendEmail({
        to: email.toLowerCase(),
        subject: "Welcome to Haridwar University ERP - Admin Account Created",
        html: `
          <h2>Welcome to Haridwar University ERP</h2>
          <p>Hello ${firstName},</p>
          ...
          <p>Best regards,<br>Haridwar University ERP System</p>
        `,
      });
    } catch (emailError) {
